"""
Batch driver for tune6 VAE outputs (Fix 1A + 1C + 2C Laplacian curvature loss).
Mirror of batch_visualize_tune2.py pointed at ghd_vae_tune6/epoch_03000.pth.
"""
import sys
...
ROOT = Path(__file__).resolve().parents[1] if Path(__file__).resolve().parent.name in ("v2", "v2_trial") else Path(__file__).resolve().parent
...
OUT_DIR = ROOT / "checkpoints" / "v2_trial" / "renders_tune6"

CHECKPOINTS = [
    ("vae_tune6", "vae", ROOT / "checkpoints" / "v2_trial" / "ghd_vae_tune6" / "epoch_03000.pth"),
]
...
@torch.no_grad()
def reconstruct_from_dataset(model, withscale, dataset, aneurysm_type, n):
...
def main():
    ...
    for label, model_kind, ckpt_path in CHECKPOINTS:
        ...
        model, withscale = build_vae(checkpoint)
            ...
            for aneurysm_type in TYPES:
                if mode == "reconstruct":
                    ghd, scale, type_tensor = reconstruct_from_dataset(
                        model, withscale, dataset, aneurysm_type, NUM_SAMPLES
                    )
                else:
