#!/bin/bash
# One clone-fit arm over the standard 10-case set. Definitive parameterized
# version -- supersedes clone_opening_run.sh / clone_arm_run.sh (kept only
# until the arms launched from them finish; editing a running bash script
# corrupts it, so new knobs go here instead).
#
# Usage: clone_arm.sh [rigid] [opening] [stageB_lr] [stageB_iters] [device]
...
cd "$ROOT"
echo "=== rigid=$LAM_RIGID open=$LAM_OPEN stageB_lr=$LR_B iters=$NITER on $DEV -> $TAG ===" > "$LOG"
