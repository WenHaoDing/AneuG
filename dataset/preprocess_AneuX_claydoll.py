"""
Variant of preprocess_AneuX.py that additionally stores the UNCLIPPED
centerline alongside the clipped one, per case.

Unlike preprocess_AneuX.py (which is dataset-switchable between ImperialNHS
and AneuX via --dataset), this script is AneuX-only — no ImperialNHS config
...
Output format (see preprocess_AneuX.py's own docstring for the ghd_root /
...
├── case_name.npy
    └── object scalar dict:
        ...
        ├── clipped_centerline: dict — identical to preprocess_AneuX.py's output
...
├── case_name_sanity.png
    └── non-interactive sanity render: GHD reconstruction mesh (gray),
        ...
        centerline branches, one color per branch from a fixed palette
        (black, blue, red, yellow) — a circle marks each branch's start
        (nearest the aneurysm) and a triangle marks its end (branch tip).
        clipped_centerline is not drawn.

Only tested on a handful of cases so far.

conda activate new
python dataset/preprocess_AneuX_claydoll.py --save-sanity
"""
...
# ── AneuX-only config (no ImperialNHS config carried in this script) ───────────
...
# Max arc length (mm) of an unclipped branch, measured from its start (nearest
# the aneurysm). Longer branches are truncated from the far/outward end, keeping
# the near-aneurysm start intact. Note: if a branch's *clipped* (post-cut) arc
# length alone already exceeds this, truncation eats into what used to be a
# guaranteed byte-identical suffix match with clipped_centerline — i.e.
# unclipped_centerline["branch_points"][i] is no longer guaranteed to be a
# superset of clipped_centerline["branch_points"][i] once this cap kicks in.
MAX_CL_LENGTH_MM = 15.0
...
def _build_unclipped_centerline(clipped_built, clipped_raw, merged_raw, affine,
                                ...
                                max_cl_length=MAX_CL_LENGTH_MM):
    ...
    for branch_id in clipped_built["branch_ids"]:
        ...
        full_pts_world = _clip_to_max_length(full_pts_world, max_cl_length)
        branches.append({
            "branch_id": branch_id,
            "group": group,
...
class ClaydollGHDPreprocessor(V2GHDPreprocessor):
    ...
    def __init__(self, *args, match_dist_threshold=UNCLIPPED_MATCH_DIST_THRESHOLD_MM,
                max_cl_length=MAX_CL_LENGTH_MM, **kwargs):
    ...
    def process_case(self, case):
        ...
        unclipped_built = _build_unclipped_centerline(
            ...
            max_cl_length=self.max_cl_length,
        )
...
def sanity_check_processed_checkpoint_with_unclipped(
    ...
    """sanity_check_processed_checkpoint (preprocess_AneuX.py), but showing only
    the unclipped centerline branches (start/end marked), not the clipped ones."""
    import matplotlib
    ...
    if ghd_reconstruct is None:
        ...
        from preprocess_AneuX import _reconstruct_with_backend
...
def _parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Preprocess AneuX GHD checkpoints into condensed .npy files, "
                    "each also carrying an unclipped_centerline dict alongside clipped_centerline."
    )
    parser.add_argument("--ghd-root", default=GHD_ROOT)
    parser.add_argument("--geometry-root", "--post-root", dest="geometry_root", default=GEOMETRY_ROOT)
    parser.add_argument("--output-dir", default=DEFAULT_OUTPUT_DIR)
    parser.add_argument("--case", action="append", dest="cases")
    parser.add_argument("--aneurysm-types", nargs="+", type=int, choices=(0, 1, 2),
                        default=list(DEFAULT_ANEURYSM_TYPES),
                        help="Which aneurysm type(s) to process (default: %(default)s).")
    ...
    parser.add_argument("--max-cl-length", type=float, default=MAX_CL_LENGTH_MM,
                        help="Max unclipped-branch arc length (mm) measured from its start "
                             "(nearest the aneurysm); longer branches are truncated from the "
                             "far/outward end (default: %(default)s).")
...
if __name__ == "__main__":
    ...
    preprocessor = ClaydollGHDPreprocessor(
        ...
        post_root=args.geometry_root,
        output_dir=args.output_dir,
        overwrite=args.overwrite,
        save_sanity=args.save_sanity,
        aneurysm_types=tuple(args.aneurysm_types),
        dataset_name="aneux",
        match_dist_threshold=args.match_dist_threshold,
        max_cl_length=args.max_cl_length,
    )
...
conda activate new
python dataset/preprocess_AneuX_claydoll.py --save-sanity
python dataset/preprocess_AneuX_claydoll.py --case p094_ERUeAB4WBg0SCwcbHgAABAEU_cut2 --save-sanity
"""
