...
For files written as "primary / fallback", use the fallback file when it
exists; otherwise use the primary file.
...
def _load_pickle_npy(path):
    # Some files were pickled with NumPy 2.x module names and loaded in 1.x envs.
...
class V2GHDPreprocessor:
    ...
    def __init__(self, ghd_root, post_root, output_dir, overwrite=False, save_sanity=True):
    ...
    def _case_paths(self, case):
        ...
        return {
            ...
            "clipped_centerline": _primary_or_fallback(post_dir, "clipped_centerline", ".npy"),
            "branch_ranking": _primary_or_fallback(post_dir, "branch_ranking", ".npy"),
...
def sanity_check_processed_checkpoint(
    ...
    """
    Reconstruct the GHD mesh and overlay the processed clipped centerlines.
    ...
    ax.set_title(f"{case} (type {aneurysm_type}: {checkpoint.get('canonical_type', 'unknown')})")
...
def _reconstruct_with_backend(checkpoint, ghd_reconstruct, aneurysm_type, device, denormalize_shape):
    ...
    if hasattr(ghd_reconstruct, "forward_as_meshes"):
        return ghd_reconstruct.forward_as_meshes(
            ghd, aneurysm_type=aneurysm_type, denormalize_shape=denormalize_shape
        )
    return case_reconstruct.ghd_forward_as_Meshes(ghd, denormalize_shape=denormalize_shape)


def _parse_args():
    parser = argparse.ArgumentParser(
        description="Preprocess v2 GHD checkpoints into one .npy file per case."
    )
    parser.add_argument("--ghd-root", default=DEFAULT_GHD_ROOT)
    parser.add_argument("--post-root", default=DEFAULT_POST_ROOT)
...
python dataset/preprocess.py --save-sanity
