"""
Batch driver for tune2 VAE outputs (mesh_weight=50, kl_weight=1e-3, lr=1e-3).
Mirror of batch_visualize_tune1.py pointed at ghd_vae_tune2/epoch_03000.pth.
"""
import sys
...
ROOT = Path(__file__).resolve().parents[1] if Path(__file__).resolve().parent.name == "v2" else Path(__file__).resolve().parent
...
OUT_DIR = ROOT / "checkpoints" / "v2" / "renders_tune2"

CHECKPOINTS = [
    ("vae_tune2", "vae", ROOT / "checkpoints" / "v2" / "ghd_vae_tune2" / "epoch_03000.pth"),
]
